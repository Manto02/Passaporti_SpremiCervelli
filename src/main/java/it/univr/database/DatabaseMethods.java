...
public abstract class DatabaseMethods {
    ...
    public int selectId(String table_name, String... parameters) throws SQLException {

        String sql = "SELECT id FROM " + table_name + " WHERE APPUNTAMENTO = ? AND LUOGO = ? AND ORA = ?";
        int id = 0;

        try(Connection conn = connect()){
            PreparedStatement pr = conn.prepareStatement(sql);
            for (int i = 0; i < parameters.length; i++) {
                pr.setString(i + 1, parameters[i]);
            }
    ...
    public String getTableName(){
        ...
        else if(this instanceof StaffDatabase)
            return "staff";
        else if(this instanceof AppointmentDatabase)
            return "appuntamenti";
        else
            return "prenotazioni";
    }
